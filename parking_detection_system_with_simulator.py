...
# Try to import SimulatedCamera from parking_simulation
try:
    from parking_simulation import SimulatedCamera
except ImportError:
...
class RealTimeParkingDetector:
    ...
    def detect_from_camera(self, camera_id: int = 0, use_simulation: bool = False, simulator=None, display: bool = True):
        """Real-time detection from camera feed or simulation"""
        if not self.detector.is_trained:
            raise ValueError("Detector not trained. Load or train a model first.")
        
        # Initialize camera based on use_simulation flag
        if use_simulation:
            if SimulatedCamera is None:
                raise ImportError("SimulatedCamera not available. Ensure parking_simulation.py is in the same directory.")
            if simulator is None:
                raise ValueError("Simulator instance required when use_simulation is True.")
            cap = SimulatedCamera(simulator)
            logger.info("Starting real-time detection from simulated camera feed")
        else:
            cap = cv2.VideoCapture(camera_id)
            if not cap.isOpened():
                raise ValueError(f"Cannot open camera {camera_id}")
            logger.info(f"Starting real-time detection from camera {camera_id}")
...
def main():
    ...
    parser.add_argument('--use-simulation', action='store_true',
                       help='Use simulated camera feed instead of real camera')
    ...
    if args.mode == 'train':
        ...
        if args.use_simulation:
            try:
                from parking_simulation import ParkingLotSimulator
                # Create and start simulator in background
                simulator = ParkingLotSimulator()
                rt_detector.set_parking_regions(simulator.get_parking_rois())
                sim_thread = threading.Thread(target=simulator.start_simulation, args=(False,))
                sim_thread.daemon = True
                sim_thread.start()
                time.sleep(2)  # Wait for simulator to initialize
                rt_detector.detect_from_camera(use_simulation=True, simulator=simulator)
            except ImportError:
                logger.error("Could not import parking_simulation. Ensure parking_simulation.py is in the same directory.")
                return
        else:
